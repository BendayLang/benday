...
impl TextDrawer {
	...
	fn get_texture(&self, text_style: &TextStyle, text: &str) -> sdl2::render::Texture {
		let TextStyle {
			// text,
			font_name: font_path,
			font_size,
	...
	pub fn text_size(&self, text_style: &TextStyle, text: &str) -> (u32, u32) {
		if text.is_empty() {
			return (0, 0);
		}
		let TextureQuery { height: y, width: x, .. } = self.get_texture(text_style, text).query();
		(x, y)
	}
