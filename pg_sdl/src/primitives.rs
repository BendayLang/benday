...
pub fn draw_arrow(canvas: &mut Canvas<Window>, camera: Option<&Camera>, color: Color,
                  start: Point2<f64>, end: Point2<f64>, width: f64) {
	if start == end { return; }
	// TODO clean up
	let x_dir = end - start;
...
pub fn get_text_size(camera: Option<&Camera>, text_drawer: &TextDrawer,
                     text: &str, font_size: f64, style: &TextStyle) -> Vector2<f64>{
	if let Some(camera) = camera {
		text_drawer.text_size(text, camera.scale() * font_size, style).cast() / camera.scale()
	}
	else { text_drawer.text_size(text, font_size, style).cast() }
}

pub fn draw_text(
	canvas: &mut Canvas<Window>, camera: Option<&Camera>, text_drawer: &TextDrawer, position: Point2<f64>,
	text: &str, font_size: f64, style: &TextStyle, align: Align,
) {
	if text.is_empty() {
		return;
	}
