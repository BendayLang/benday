...
impl Rect {
	...
	pub fn width(&self) -> f64 { self.size.x }
	pub fn height(&self) -> f64 { self.size.y }
	
	pub fn top(&self) -> f64 { self.position.y + self.size.y }
	pub fn v_mid(&self) -> f64 { self.position.y + self.size.y * 0.5}
	pub fn bottom(&self) -> f64 { self.position.y }
	
	pub fn left(&self) -> f64 { self.position.x }
	pub fn h_mid(&self) -> f64 { self.position.x + self.size.x * 0.5 }
	pub fn right(&self) -> f64 { self.position.x + self.size.x }
	
	pub fn top_left(&self) -> Point2<f64> { self.position + Vector2::new(self.size.x * 0., self.size.y) }
	pub fn mid_top(&self) -> Point2<f64> { self.position + Vector2::new(self.size.x * 0.5, self.size.y) }
	pub fn top_right(&self) -> Point2<f64> { self.position + self.size }
	pub fn mid_left(&self) -> Point2<f64> { self.position + Vector2::new(0., self.size.y * 0.5) }
	pub fn center(&self) -> Point2<f64> { self.position + self.size * 0.5 }
	pub fn mid_right(&self) -> Point2<f64> { self.position + Vector2::new(self.size.x, self.size.y * 0.5) }
	pub fn bottom_left(&self) -> Point2<f64> { self.position }
