...
impl Manager {
	...
	fn update_hovered_widget(&mut self, new_hovered_widget_id: Option<WidgetId>) {
		if let Some(old_hovered_widget_id) = self.hovered_widget_id {
			if if let Some(new_hovered_widget_id) = new_hovered_widget_id {
				if self.get_widget(&new_hovered_widget_id).get_base().parent_id == Some(old_hovered_widget_id) {
					false
				} else { true }
			} else { true } {
				let widget = self.widgets.remove(&old_hovered_widget_id).unwrap();
				widget.borrow_mut().on_unhover(self);
				self.widgets.insert(old_hovered_widget_id, widget);
				self.get_widget_mut(&old_hovered_widget_id).get_base_mut().hovered = false;
				let option_parent_id = self.get_widget(&old_hovered_widget_id).get_base().parent_id.clone();
				if let Some(parent_id) = option_parent_id {
					let widget = self.widgets.remove(&parent_id).unwrap();
					widget.borrow_mut().on_unhover(self);
					self.widgets.insert(parent_id, widget);
					self.get_widget_mut(&parent_id).get_base_mut().hovered = false;
				}
	...
	pub fn get<T: Widget>(&self, id: &WidgetId) -> Ref<T> {
		Ref::map(self.widgets.get(id).unwrap().borrow(), |x| x.as_ref().downcast_ref::<T>().unwrap())
	}
	
	pub fn get_mut<T: Widget>(&self, id: &WidgetId) -> RefMut<T> {
		RefMut::map(self.widgets.get(id).unwrap().borrow_mut(), |x| x.as_mut().downcast_mut::<T>().unwrap())
	}
