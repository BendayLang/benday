...
use crate::blocs::bloc::NewBlocStyle;
...
impl Bloc {
	...
	const TOP_BOX_SIZE: Vector2<f64> = Vector2::new(100., 25.);
	const TOP_BOX_MARGIN: f64 = 3.;
	const HOVER_ALPHA: u8 = 20;
	// const COLOR: Color = hsv_color(330, 0.3, 1.);
	const PRINT_TEXT_SIZE: Vector2<f64> = Vector2::new(30., 10.); // size of "PRINT" text
	...
	pub fn new_bloc(color: Color, position: Point2<f64>, bloc_type: BlocType) -> Self {
		let (slots, slots_positions, sequences, sequences_positions, get_size, style): (
			...
			BlocType::Print => (
				...
				Box::new(|bloc: &Bloc, slot_id: usize| match slot_id {
					0 => Vector2::new(Self::PRINT_TEXT_SIZE.x + Self::INNER_MARGIN, 0.) + Vector2::new(1., 1.) * Self::MARGIN,
					_ => {
						Vector2::new(
							Self::PRINT_TEXT_SIZE.x + Self::INNER_MARGIN,
							bloc.slots[0].get_size().y + Self::INNER_MARGIN,
						) + Vector2::new(1., 1.) * Self::MARGIN
					}
				}),
				...
				Box::new(|bloc: &Bloc| {
					...
					Vector2::new(width + Self::PRINT_TEXT_SIZE.x + Self::INNER_MARGIN, height + Self::INNER_MARGIN)
						+ Vector2::new(2., 2.) * Self::MARGIN
				}),
				NewBlocStyle::new(color, Self::RADIUS),
			),
			BlocType::IfElse => (
				vec![Slot::new(color, "condition".to_string())],
				Box::new(|_bloc: &Bloc, _slot_id: usize| {
					Vector2::new(Self::IF_TEXT_SIZE.x + Self::INNER_MARGIN, 0.) + Vector2::new(1., 1.) * Self::MARGIN
				}),
				vec![Sequence::new(color), Sequence::new(color)],
				Box::new(|bloc: &Bloc, sequence_id| {
					let width = Self::IF_TEXT_SIZE.x + bloc.slots[0].get_size().x + 2. * Self::INNER_MARGIN;
					...
					Vector2::new(width, height) + Vector2::new(1., 1.) * Self::MARGIN
				}),
				Box::new(|bloc: &Bloc| {
					...
					Vector2::new(width, height) + Vector2::new(2., 2.) * Self::MARGIN
				}),
				NewBlocStyle::new(color, Self::RADIUS),
			),
		...
		bloc.base.rect.size = (*bloc.get_size)(&bloc);
		bloc
	}
	...
	pub fn set_position(&mut self, position: Point2<f64>) {
		self.base.rect.position = position
	}
	...
	pub fn update_child_position(&mut self, blocs: &mut HashMap<u32, Bloc>) {
		self.slots.iter_mut().for_each(|slot| slot.update_child_position(self.base.rect.position, blocs));
		self.sequences.iter().for_each(|sequences| sequences.update_child_position(self.base.rect.position, blocs))
	}
	...
	pub fn collide_element(&self, point: Point2<f64>) -> Option<BlocElement> {
		...
		for (slot_id, slot) in self.slots.iter().enumerate() {
			if slot.get_rect().collide_point(point - self.base.rect.position.coords) {
				return Some(BlocElement::Slot(slot_id));
			}
		}

		for (sequence_id, sequence) in self.sequences.iter().enumerate() {
			if sequence.get_rect().collide_point(point - self.base.rect.position.coords) {
	...
	pub fn collide_container(&self, rect: Rect) -> Option<(BlocContainer, f64)> {
		...
		self.slots.iter().enumerate().for_each(|(slot_id, slot)| {
			if slot.get_rect().collide_rect(rect.translated(-self.base.rect.position.coords)) && !slot.has_child() {
				let new_ratio = slot.get_ratio(rect.translated(-self.base.rect.position.coords));
				if new_ratio > ratio {
