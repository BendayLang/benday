...
impl Bloc {
	...
	pub fn draw(&self, canvas: &mut Canvas<Window>, text_drawer: &mut TextDrawer) {
		...
		let mut surface =
			Surface::new(self.rect.width(), self.rect.height(), texture_creator.default_pixel_format()).unwrap();

		surface.fill_rect(rect!(0, 0, self.rect.width(), self.rect.height()), self.color).unwrap();
...
pub fn set_child(child_id: u32, parent_id: u32, blocs: &mut HashMap<u32, Bloc>) {
	...
	let (child_width, child_height) = child_size;
	{
		...
		parent_bloc.set_size((parent_bloc.rect.width() + child_width, parent_bloc.rect.height() + child_height));
	}
