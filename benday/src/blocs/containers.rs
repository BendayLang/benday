...
impl Slot {
	...
	pub fn draw(&self, canvas: &mut Canvas<Window>, text_drawer: &TextDrawer, camera: &Camera, selected: bool, hovered: bool) {
		if self.child_id.is_none() { self.text_box.draw(canvas, text_drawer, camera, selected, hovered); }
	}
...
impl Sequence {
	...
	fn get_child_position(&self, place: usize) -> Vector2<f64> {
		if place == self.childs_ids.len() { Vector2::new(0.0, self.size.y) } else { self.childs_positions[place] }
	}
	...
	/// Affiche la séquence.
	pub fn draw(
		&self, canvas: &mut Canvas<Window>, camera: &Camera, position: Point2<f64>,
	...
	pub fn draw_hover(&self, canvas: &mut Canvas<Window>, camera: &Camera, position: Point2<f64>, place: usize) {
		...
		camera.fill_rounded_rect(
			canvas,
			hovered_color,
			position + self.position + place_position,
			size,
			radius,
		);
		canvas.set_blend_mode(BlendMode::None);
