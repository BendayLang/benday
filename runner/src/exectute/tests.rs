...
use crate::exectute::{
	console::Console,
	execute::{execute_node, ActionType},
	VariableMap,
};
use models::{
	ast::*,
...
fn test(
	...
	expected_variables: Option<VariableMap>, expected_return_value: Option<AstResult>, expected_actions: Vec<ActionType>,
) {
	let (stdout_result, actions_result) = if let Some(mut variables) = basic_variables {
		...
		let runner_result = execute_node(&ast, &mut variables, id_path, &mut console, &mut actions);
		(console, actions)
	} else {
		runner(&ast)
	};
...
fn should_do_nothing_when_empty_sequence() {
	...
	test(ast, None, None, None, None, vec![ActionType::Goto(0), ActionType::Return(Ok(None))]);
}
...
fn should_assign_variable_and_print_it() {
	...
	test(
		...
		vec![
			ActionType::Goto(0),
			ActionType::Goto(1),
			ActionType::CheckVarNameValidity(Ok(())),
			ActionType::Goto(2),
			ActionType::EvaluateRawText,
			ActionType::Return(Ok(Some(ReturnValue::Int(42)))),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(42) },
			ActionType::Return(Ok(None)),
			ActionType::Goto(3),
			ActionType::GetArgs,
			ActionType::Goto(4),
			ActionType::EvaluateRawText,
			ActionType::Return(Ok(Some(ReturnValue::Int(42)))),
			ActionType::CallBuildInFn("print".to_string()),
			ActionType::PushStdout("42".to_string()),
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
		],
	);
...
fn should_return_value_when_raw_text() {
	...
	test(
		...
		vec![ActionType::Goto(0), ActionType::EvaluateRawText, ActionType::Return(Ok(Some(ReturnValue::Int(42))))],
	);
}
...
fn should_return_when_raw_text_and_stop_there() {
	...
	test(
		...
		vec![
			...
			ActionType::Goto(1),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(42)),
			ActionType::return_some(ReturnValue::Int(42)),
		],
	);
}
...
fn should_print_variable_in_a_while_loop() {
	...
	test(
		...
		vec![
			...
			ActionType::Goto(1),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(1)),
			ActionType::Goto(200),
			ActionType::Goto(4),
			ActionType::GetArgs,
			ActionType::Goto(5),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::String("x=0 !".to_string())),
			ActionType::CallBuildInFn("print".to_string()),
			ActionType::PushStdout("x=0 !".to_string()),
			ActionType::Return(Ok(None)),
			ActionType::Goto(2),
			ActionType::CheckVarNameValidity(Ok(())),
			ActionType::Goto(3),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(1)),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(1) },
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
			ActionType::ControlFlowEvaluateCondition,
			ActionType::Goto(1),
			...
			ActionType::Goto(5),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::String("x=1 !".to_string())),
			ActionType::CallBuildInFn("print".to_string()),
			ActionType::PushStdout("x=1 !".to_string()),
			ActionType::Return(Ok(None)),
			ActionType::Goto(2),
			ActionType::CheckVarNameValidity(Ok(())),
			ActionType::Goto(3),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(2)),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(2) },
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
			ActionType::ControlFlowEvaluateCondition,
			ActionType::Goto(1),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(0)),
			ActionType::Return(Ok(None)),
		],
	);
}
...
fn sould_reassign_variable_if_condition_is_true() {
	...
	test(
		...
		vec![
			...
			ActionType::Goto(2),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(10)),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(10) },
			ActionType::Return(Ok(None)),
			ActionType::Goto(3),
			ActionType::ControlFlowEvaluateCondition,
			ActionType::Goto(4),
			...
			ActionType::Goto(12),
			ActionType::EvaluateRawText,
			ActionType::Return(Ok(Some(ReturnValue::Int(13)))),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(13) },
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
...
fn should_return_math_expression_result() {
	...
	test(
		...
		vec![
			...
			ActionType::Goto(3),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(-38)),
			ActionType::return_some(ReturnValue::Int(-38)),
		],
	);
}
...
fn should_reassign_variable() {
	...
	test(
		...
		vec![
			...
			ActionType::Goto(4),
			ActionType::EvaluateRawText,
			ActionType::return_some(ReturnValue::Int(24)),
			ActionType::AssigneVariable { key: ("x".to_string(), 0), value: ReturnValue::Int(24) },
			ActionType::Return(Ok(None)),
			ActionType::Return(Ok(None)),
		],
	);
